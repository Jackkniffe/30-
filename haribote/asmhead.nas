...
; 画面模式的切换
		...
		MOV		AX,0x4f02
		INT		0x10
		MOV		BYTE [VMODE],8	; 记录下画面模式（参照C语言）
		MOV		AX,[ES:DI+0x12]
		MOV		[SCRNX],AX
		MOV		AX,[ES:DI+0x14]
		MOV		[SCRNY],AX
...
;	随后进行PIC的初始化
		...
		MOV		AL,0xff
		OUT		0x21,AL
		NOP						; 如果连续执行OUT命令，有些机种无法正常运行
		OUT		0xa1,AL

		CLI						; 禁止CPU级别的中断

; 为了让CPU能访问1MB以上的内存空间设定A20GATE
...
waitkbdout:
		IN		 AL,0x64
		AND		 AL,0x02
		IN		 AL,0x60 		; 空读(为了情空数据接收缓冲区中的垃圾数据)
		JNZ		waitkbdout		; AND的结果不是0就跳转到waitkbdout
		RET
...
memcpy:
		...
		ADD		EDI,4
		SUB		ECX,1
		JNZ		memcpy			; 减法运行的结果如果不是0，就跳到memcpy
		RET
; memcpy 
