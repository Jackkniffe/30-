...
_asm_hrb_api:
		...
		PUSHAD		; 用于向hrb_api的传值PUSH
		MOV		AX,SS
		MOV		DS,AX		; 将操作系统用段地址存入DS和CS
		MOV		ES,AX
		CALL	_hrb_api
...
;	下面调整栈，是RETF时跳转到应用程序
		...
		PUSH	ECX				; 应用程序用的CS
		PUSH	EAX				; 应用程序用的EIP
		RETF                    ;本质作用是从栈中将地址POP出来，然后JMP到该地址
;	应用程序结束后不回返回这里
