...
struct SHEET *sheet_alloc(struct SHTCTL *ctl)
{
	...
	for (i = 0; i < MAX_SHEETS; i++) {
		if (ctl->sheets0[i].flags == 0) {
			...
			sht->height = -1; /* 标记图层为隐藏状态 */
			sht->task = 0;	/* 自動で閉じる機能を使わない */
			return sht;
		}
	}
	return 0;	/* 表示所有图层都处于使用状态 */
}
/*设置图层的需要显示的内容，图层的大小，图层的颜色*/
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
	...
	for (h = h0; h <= h1; h++) {
		...
		/* 使用vx0-vy1，倒推bx0-by1
           原理：
           1.当一个图层发生变化的时候，它的变化范围会影响到其他图层的重新绘制的范围
           2.其他图层只会重新绘制被影响的区域
           3.自己悟
		*/
		bx0 = vx0 - sht->vx0;
...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	sht->height = height; /*设置新的高度*/

	/* 需要对sheets[]的图层顺序重新排列 */
	if (old > height) {	/*高度比以前低(图层下降)*/
		if (height >= 0) {
			/*把height到old中间的图层往上提*/
			...
			ctl->top--; /*由于显示中的图层减少一个，所以最上面的图层下降*/
			/*刷新显示缓冲区*/
		...
		if (old >= 0) {
			...
			ctl->sheets[height] = sht;
		} else {	/*old<0 表明此次图层状态变化是由隐藏状态变为显示状态,需在sheets[]中添加一项数据*/
			/* 将height级以上的数据向上升 */
			...
			ctl->sheets[height] = sht;
			ctl->top++; /* 由于显示的图层添加了一个，所以最上面的图层高度加1 */
		}
		//刷新显示缓冲区
